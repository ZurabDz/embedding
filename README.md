...
| `geo-distill` (imports as `geo_distill`) | [`geo_distill/`](geo_distill/) | distills Gemini teacher embeddings into a student — either a tiny from-scratch model or the pretrained MLM encoder fine-tuned |
...
consumes both via `--mlm-checkpoint` / `--tokenizer`.
...
- `python -m geo_distill <stage> …` or the `geo-distill` script — the
  distillation pipeline: `data | tokenizer | teacher | synthetic-teacher |
  train | eval` (see [`geo_distill/README.md`](geo_distill/README.md)).
